...
function loadTabs() {
    ...
    if (storedTabs && storedTabs.length) {
        storedTabs.forEach(tab => addTab(tab.title, tab.id));
    } else {
        addTab("Tab 1"); // Create the first tab by default
    }
}
...
function addTab(title, id) {
    ...
    const tabId = id || `tab-${++currentTabId}`;
    tab.className = 'tab';
    tab.id = tabId;

    const titleSpan = document.createElement('span');
    titleSpan.textContent = title || `Tab ${currentTabId}`;
    titleSpan.contentEditable = true;
    titleSpan.classList.add('editable');
    titleSpan.onblur = updateTabTitle;
    titleSpan.onkeypress = function(event) {
        if (event.key === "Enter") {
    ...
    tab.onclick = () => switchTab(tabId, titleSpan.textContent);
    tabsContainer.insertBefore(tab, document.getElementById('addTab'));

    if (!id && currentTabId === 1) {
...
function switchTab(tabId, title) {
    ...
    editor.textContent = `Content for ${title}`;
}

function updateTabTitle() {
    if (this.textContent.trim() === "") {
        this.textContent = "Untitled Tab";
    }
    switchTab(this.parentElement.id, this.textContent);
    saveTabs();
}

function saveTabs() {
    const tabs = [...document.querySelectorAll('.tab')].map(tab => {
        return { title: tab.firstChild.textContent, id: tab.id };
    });
